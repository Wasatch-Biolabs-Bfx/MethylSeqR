...
This vignette will walk you through a typical workflow using a database created by Wasatch Biolabs (https://www.wasatchbiolabs.com/). This database contains results from a nanopore sequencing run. Within the directory is an initial set of folders correlating to each sample. Within each sample folder is a list of chromosomal folders holding data for each chromosome. The example database is BLANK and comes from BLANK... This example contains results from...
