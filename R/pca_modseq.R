pca_modseq <- function(modseq_dat) {
  ...
  if (regional_dat) {
    ...
    test_wide <- modseq_dat |>
      ...
      pivot_wider(names_from = sample_name, values_from = mean_mh_frac) |> na.omit()
  } else {
    # Aggregate mean_mh_frac by sample and region_name
    test_wide <- modseq_dat %>%
      slice_sample(n = 500000) %>%
      mutate(chr_pos = paste(chrom, ref_position, sep = "_")) %>%
      pivot_wider(id_cols = chr_pos, names_from = sample, values_from = mh_frac) %>%
      na.omit()
  }
  ...
  geom_point(size = 3) +
  theme_minimal() +
  labs(title = "PCA Plot of Methylation Data", x = "Principal Component 1", y = "Principal Component 2")
}
