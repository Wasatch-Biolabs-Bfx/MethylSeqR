...
## Paradigm 
Begin with CH3 files using the `make_pos_db()` function to generate position databases. After that, you can use `qc_wrapper()` to visually assess positional data, or summarize by region (`summarize_regions()`) or windows (`summarize_windows`).
